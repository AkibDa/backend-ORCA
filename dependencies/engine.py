...
# Current ORCA agent/model repository.
# backend/dependencies/engine.py
#        parents[0] = dependencies
#        parents[1] = backend
#        parents[2] = ORCA
# Therefore:
# W:\ORCA\agent-orchestration
AGENT_DIR = Path(__file__).resolve().parents[2] / "agent-orchestration"

if not AGENT_DIR.exists():
    raise RuntimeError(
        f"agent-orchestration directory not found: {AGENT_DIR}"
    )
...
def create_orca_engine() -> OrcaOrchestrator:
    ...
    # Pre-warm all ML models synchronously to avoid initialization
    # races during concurrent agent execution.
    try:
